...
router.post('/', checkExpiredCode, (req, res) => {
    Verification.findOne({ type: 'signup', email: req.body.email, code: req.body.code}, '_id', (err, doc) => {
        if(err){
            res.json(err);
        }
        if(doc == null){
...
router.post('/resendCode', checkExpiredCode, (req, res) => {
    User.findOne({ email: req.body.email }, '_id', (err, doc) => {
        ...
        if(doc != null){
            ...
            Verification.find({ email: req.body.email, type: 'signup' }, '_id', (err, doc) => {
                if(err) {
                    res.json(err);
                }
                if(doc != null){
                ...
                let mailOptions = {
                    ...
                    to: req.body.email,
                    subject: 'Verification code for GRING',
                    text: `Your GRING verification code is ${code}`
                };
